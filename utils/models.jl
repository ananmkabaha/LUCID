...
function get_nn(model_path, model_name, dim, c, dataset)
    ...
    elseif occursin("CNN",model_name)
        is_conv = true
        stride_to_use_1 = 1
        stride_to_use_2 = 1
        conv_filters = 6
...
function get_nn_hyper(model_path, model_name, dim, c, dataset, hypers_dir_path, is_deps, wi, token)
    ...
    if occursin("2x",model_name)
        ...
        model_pth = myunpickle(hypers_dir_path*"/hypernetwork_min_box_"*string(wi)*"_"*string(token)*".p")
        dict_min = Dict{String,Any}( "fc1/weight"=>model_pth[1],"fc1/bias" => reshape(model_pth[2],(1,length(model_pth[2]))),
             "fc2/weight"=>model_pth[3],"fc2/bias" => reshape(model_pth[4],(1,length(model_pth[4]))),
            "fc3/weight"=>model_pth[5],"fc3/bias" => reshape(model_pth[6],(1,length(model_pth[6]))))
        model_pth = myunpickle(hypers_dir_path*"/hypernetwork_max_box_"*string(wi)*"_"*string(token)*".p")
