...
$(FIELDS)
"""
mutable struct HyperConv2d{T<:JuMPReal,U<:JuMPReal,V<:Integer} <: Layer
    filter::Array{T,4}
    bias::Array{U,1}
    stride::V
    padding::Padding
    ...
    function HyperConv2d{T,U,V}(
        ...
        return new(filter, bias, stride, padding, filter2, bias2, stride2, padding2)
    end

end

function HyperConv2d(
    filter::Array{T,4},
    bias::Array{U,1},
    stride::V,
    padding::Padding,
    ...
    HyperConv2d{T,U,V}(filter, bias, stride, padding,filter2, bias2, stride2, padding2)
end
...
function HyperConv2d(
    ...
    HyperConv2d{T,U,V}(filter, bias, stride, SamePadding(),filter2, bias2, stride2, SamePadding())
end

function HyperConv2d(filter::Array{T,4}, bias::Array{U,1},filter2::Array{T,4}, bias2::Array{U,1}) where {T<:JuMPReal,U<:JuMPReal}
    HyperConv2d(filter, bias, 1, SamePadding(),filter2, bias2, 1, SamePadding())
end
...
and zero bias.
"""
function HyperConv2d(filter::Array{T,4},filter2::Array{T,4}) where {T<:JuMPReal}
    ...
    HyperConv2d(filter, bias,filter2, bias2)
end

function check_size(params::HyperConv2d, sizes::NTuple{4,Int})::Nothing
    check_size(params.filter, sizes)
    check_size(params.bias, (sizes[end],))
end

function Base.show(io::IO, p::HyperConv2d)
...
"""
function Hyperconv2d(input::Array{T,4}, params::HyperConv2d{U,V}) where {T<:JuMPReal,U<:JuMPReal,V<:JuMPReal}
    ...
    output_low = Array{W}(undef, output_size)

    @nloops 4 i output_low begin
        (@nref 4 output_low i) = params.bias[i_4]
        @nloops 3 j filter begin
            ...
            if checkbounds(Bool, input, input_index...)
                ...
                (@nref 4 output_low i) = add_to_expression!(
                    (@nref 4 output_low i),
                    input[input_index...],
    ...
    filter = params.filter2
    stride = params.stride2
    padding = params.padding2
    ...
    W = Base.promote_op(+, V, Base.promote_op(*, T, U))
    output_high = Array{W}(undef, output_size)

    @nloops 4 i output_high begin
        (@nref 4 output_high i) = params.bias2[i_4]
        @nloops 3 j filter begin
            x = (i_2 - 1) * stride + j_1 - filter_height_offset
            y = (i_3 - 1) * stride + j_2 - filter_width_offset
            input_index = (i_1, x, y, j_3)
            if checkbounds(Bool, input, input_index...)
                # Effectively zero-padding the input.
                (@nref 4 output_high i) = add_to_expression!(
                    (@nref 4 output_high i),
                    input[input_index...],
                    filter[j_1, j_2, j_3, i_4],
                )
            end
    ...
    println(size(output_low))
    println(size(output_high))
    input_range = CartesianIndices(size(output_high))
    output = map(
        i -> @variable(
            model,
            lower_bound = -1000000,
    ...
    @constraint(model,output .<= output_high)
    @constraint(model,output .>= output_low)

    return output
end

(p::HyperConv2d)(x::Array{<:JuMPReal,4}) = Hyperconv2d(x, p)
