...
function tight_bound_helper(m::Model, bound_type::BoundType, objective::JuMPLinearType, b_0::Number
    ...
    set_optimizer_attribute(m, "Threads", 2)
...
function relu(x::T, l::Real, u::Real)::JuMP.AffExpr where {T<:JuMPLinearType}
	...
	nueron_counter += 1
	use_relaxations_cnt += 1
    #println("me_th",me_th)
    if u <= 0
        # rectified value is always 0
        model = owner_model(x)
        x_rect = @variable(model)
    	set_name(x_rect,string(network_version,"_x_layer_",string(layer_counter),"_neuron_",string(nueron_counter)))
        a = @variable(model, binary = true)
		set_name(a,string(network_version,"_a_layer_",string(layer_counter),"_neuron_",string(nueron_counter)))
    ...
    elseif l >= 0
        ...
        #println(length(av))
        push!(layers_info_dict,(layer_counter,nueron_counter)=>(u,l,length(av)))

        diff_to_use = diff_
        if network_version == "hyper"
            if length(size(diff_to_use))>2
                diff_to_use = diff_to_use |> Flatten([1, 2, 3, 4])
            end
        end

        x_rect = @variable(model)
    	set_name(x_rect,string(network_version,"_x_layer_",string(layer_counter),"_neuron_",string(nueron_counter)))
        if length(diff_to_use) == 0 || diff_to_use[nueron_counter]>=me_th
            a = @variable(model, binary = true)
			set_name(a,string(network_version,"_a_layer_",string(layer_counter),"_neuron_",string(nueron_counter)))
...
function relu(
    ...
    #if network_version == "hyper" && length(diff_)>0
    #    println("----------------------------")
    #    println(diff_)
    #end
	...
	if !show_progress_bar
        ...
        if network_version == "org"
            append!(all_bounds_of_original,[[u,l]])
        end
        return relu.(x, l, u)
    else
        ...
        notice(MIPVerify.LOGGER, "$reluinfo")
        u_for_spread = u
        l_for_spread = l
